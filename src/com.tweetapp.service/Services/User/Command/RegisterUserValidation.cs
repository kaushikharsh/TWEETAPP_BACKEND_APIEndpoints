namespace com.tweetapp.service
{
    ...
    public class RegisterUserValidation : AbstractValidator<RegisterUserModel>
    {
        public RegisterUserValidation()
        {
            ...
            RuleFor(x => x.Password).NotEmpty().NotNull().Must(ValidatePassword).WithMessage("Invalid password (length must be greater than 5). ");
            RuleFor(x => x.FirstName).NotEmpty().NotNull().Must(ValidateFirstName).WithMessage("Invalid First Name. ");
            RuleFor(x => x.Gender).NotEmpty().NotNull().Must(ValidateGender).WithMessage("Invalid Gender. Please [(input)- M for Male and - F for Female and - O for Other]. ");
            ...
            RuleFor(x => x.MobilePhone).NotEmpty().NotNull().Length(1,10).WithMessage("Invalid Phone Number. ");
        }
